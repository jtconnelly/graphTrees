...
namespace _
{
    ...
    // this node
    template<typename T>
    std::string BTreeNode<T>::traverse() const
    {
        std::string ans;
...
// greater than or equal to k
    ...
    template<typename T>
    bool BTreeNode<T>::remove(const T& k)
    {
        int idx = findKey(k);
    ...
    template<typename T>
    bool BTreeNode<T>::removeFromLeaf(int idx)
    {
    ...
    template<typename T>
    bool BTreeNode<T>::removeFromNonLeaf(int idx)
    {
...
{
    template<typename T>
    std::string BTree<T>::traverse() const
    {
        std::string ans;
        if (root != nullptr)
            ans = root->traverse();
    ...
    void BTree<T>::insert(const T& k)
    {
        ...
        else // If tree is not empty
        {
            ...
            if (root->n == 2 * t - 1)
            {
                ...
                _::BTreeNode<T>* s = new _::BTreeNode<T>(t, false);
    ...
    bool BTree<T>::remove(const T& k)
    {
        ...
        bool ans = root->remove(k);

        // If the root node has 0 keys, make its first child as the new root
        //  if it has a child, otherwise set root as NULL
        if (root->n == 0)
        {
            _::BTreeNode<T>* tmp = root;
            if (root->leaf)
                root = NULL;
            else
                root = root->C[0];
...
//template class Boots::BTreeNode<int>;
template class Boots::BTree<int>;